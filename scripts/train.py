...
from challenge import builder, compute, preprocessing, stain, utils
...
def parse_args() -> argparse.Namespace:
    ...
    # Data
    parser.add_argument("--train", type=Path, default="data/train.h5", help="Path to training .h5 file.")
    parser.add_argument("--val", type=Path, default="data/val.h5", help="Path to validation .h5 file.")
    parser.add_argument("--shuffle", action="store_true")
    parser.add_argument("--HEDJitter", type=float, nargs=2, default=[0, 0], help="Theta parameter.")
    parser.add_argument("--HEAug", type=float, nargs=3, default=[0, 0, 0], help="Theta parameter.")
    parser.add_argument(
        "--normalizer",
        nargs=2,
    ...
    parser.add_argument("--lora", type=int, nargs=2, default=[0, 0], help="LoRa parameters")
    parser.add_argument(
        "--batch-size",
        type=int,
        default=512,
    ...
    parser.add_argument("--lr", type=float, default=1e-3, help="Initial learning rate for the optimiser.")
    parser.add_argument(
        "--weight-decay",
        type=float,
        default=1e-4,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Apply stain-colour augmentation during training (uses STAIN_TRANSFORMS).",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.4,
        help="Strength for ColorJitter (brightness/contrast/saturation). Set to 0 to disable.",
    )
    parser.add_argument(
        "--hue-jitter",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=[0, 0],
        help="Alpha parameter for CutMix Beta distribution. Set to 0 to disable CutMix .",
    )
    parser.add_argument(
        "--mix-prob",
        type=float,
...
def build_train_transform(args) -> T.Compose:
    ...
    if args.HEAug[0] > 0:
        steps.append(T.RandomApply([stain.HEAugmentor(args.HEAug[0], args.HEAug[1])], p=args.HEAug[2]))
...
def build_val_transform() -> T.Compose:
    ...
    val_transform = T.Compose([T.Normalize(mean=MEAN, std=STD)])
